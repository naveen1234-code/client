...
export default function BodyProgressPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [list, status] = await Promise.all([getBodyProgressMe(), getBodyProgressStatusMe()]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await updateBodyProgressMe(editingId, form);
      } else {
        await uploadBodyProgressMe(form);
      }
      ...
      <PageTransition>
        <main className="min-h-screen bg-black px-4 py-10 text-white">
          <div className="mx-auto max-w-5xl">
            <h1 className="text-3xl font-black uppercase tracking-tight">Body Progress</h1>
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-400">
                Track your measurements and photos. Baseline vs current status is shown below.
              </p>
            </div>
            <button
          ...
          <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-400">
                  Add photos (front/side/back) and update your measurements.
                </p>
              </div>
              {editingId ? (
                <button
          ...
          <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            {entries.length === 0 ? (
              <div className="mt-6 rounded-2xl border border-white/10 bg-black/40 p-6 text-sm text-gray-400">
                No entries yet. Add your first baseline entry above.
              </div>
            ) : (
