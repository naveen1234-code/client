...
export default function DoorControlPage() {
  ...
  const handleUnlockDoor = async () => {
    ...
    try {
      ...
      setStatus("Unlock command sent ✅ Door should open now.");
    } catch (err: any) {
          ...
          <section className="mt-8 flex flex-1 flex-col rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-2xl">
            ...
            <div className="my-8 flex flex-1 items-center justify-center">
              <button
                ...
                disabled={unlocking}
                className="flex h-56 w-56 items-center justify-center rounded-full border border-red-400/40 bg-red-600 text-center text-2xl font-black uppercase leading-tight tracking-wide text-white shadow-[0_0_70px_rgba(220,38,38,0.45)] transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-60"
              >
